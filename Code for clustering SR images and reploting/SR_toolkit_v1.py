...
def user_input():
    ...
    ##### input #####
    directory = r"C:\Users\DRW\Desktop\code testing\Superrestest\a\0"
    pixel_size = 131.5
...
def easy_numbers(results, d):
    ...
    for x in range(0, cluster_num+1):
...
def nearest_neighbour(real_locs, directory, pixel_size):
    ...
    for cluster_num in range(int(np.amax(real_locs[:,15]))+1):
        ...
        try:
            ...
            for x in range(cluster_xy.shape[0]):
...
def skeletonize(directory, threshold, gauss_sigma, cluster_size, txtf):
    ...
    for label in range(1, max_val + 1):             # Eric - set the range from 1 to max_value+1
...
def save_data(final_counts, ecc, d): 
    ...
    with open(fi, 'w') as f:
        for x1, x2 in list(zip(final_counts, ecc)):
