...
def user_input():
    ...
    Known bugs:
        1. ///Fixed///// 20170805 - length analysis drops a cluster/FOV at some
                                    point.
        2. ///Discard/// 20170805 - the width=precision image may not be right. 
                                    Not sure if the precision should be SD,
                                    FWHM or what. Currently treating as if it
                                    is SD. # Eric - all_fits_with_header.txt is
                                    supposed to input into imageJ and generate 
                                    this image using the GDSC SMLM results
                                    manager. The code is available in 
                                    'Dropbox/Shared Anlaysis/code/Generate_SR.
                                    py'. It needs to be run in ImageJ Jython 
                                    platform.
                                    
        3. ///Fixed///// 20170807 - Can't properly seperate the analysis
        4. ///Fixed///// 20170808 - fibril length analysis image is rotated
        5. ///Discard/// 20170808 - Summary of summary is wrong
        6. ///Fixed///// 20170809 - Memory overflown on large clusters 
                                    (over 50000 localisations)
        7. ///20170809 - Eric is this your code or the module?
                         Future-proof the warning during DBSCAN: 
                         C:\Users\DRW\Anaconda2\lib\site-packages\sklearn\
                         utils\validation.py:395: DeprecationWarning: Passing 
                         1d arrays as data is deprecated in 0.17 and will raise
                         ValueError in 0.19. Reshape your data either using 
                         X.reshape(-1, 1) if your data has a single feature or 
                         X.reshape(1, -1) if it contains a single sample.
                         Warning suppressed on line 100.
    ...
    directory = r"E:\20170922_Harvard_samples__2017-09-22_17-20-25"
    pixel_size = 131.5
...
#       Suppress warnings
    ...
    for path in paths:
        ...
        if extracted_data != False:
            if commands[3] == True:             # save images
            ...
            if commands[4] == True:             # NN analysis
                ...
                extracted_data = extracted_data + [np.nan]*6  # Eric - fill array
            if commands[7] == True:             # all fits with header
                headerise(path)
            if commands[5][0] == True:          # length analysis
                ...
                extracted_data = extracted_data + [np.nan]*5  # Eric - fill array
...
def cluster_analysis(directory, dbscan_params):
    ...
    for path in path_list:
        ...
        try:
            ...
            fitarray.to_csv(os.path.join(path, 'DBSCAN_Results.csv'),
                       sep = '\t')
        except ValueError:
...
def output_images(results, pixel_size, path):
...
def easy_numbers(results, d):
    ...
    # This just gets numbers out of DBSCAN_Results.csv that need no analysis
...
def nearest_neighbour(real_locs, directory, pixel_size):
    ...
    for cluster_num in xrange(int(np.amax(real_locs[:,15]))+1):
        ...
        try:
            # Eric - change the NN analysis to make it more space efficient
            nbrs = NearestNeighbors(n_neighbors=2, algorithm='auto').fit(cluster_xy)   
            distances, indices = nbrs.kneighbors(cluster_xy)
            distances = distances[:,1]
            nn_dist.append(np.mean(distances)*pixel_size)
            thresh_distance = np.mean(distances)*5 # 5*NN dist for each cluster
            neighbors_per_loc = []
...
def skeletonize(directory, threshold, gauss_sigma, cluster_size, txtf):
    ...
    # Eric - set the range from 1 to max_value+1
    # and then add 1 to the original label (which starts from 0)
    for label in xrange(1, max_val + 1):             
        a = arr[np.where(arr[:,0] + 1 == label)]     
    ...
    # Eric- add transpose to rotate it back to the correct position
    scipy.misc.imsave(sp, skele2.transpose())   
    ...
    for i in ii:
        ...
        for k in nearby(i):
            try:
                ...
                pass
        # Eric - Add nearby pixels to expand the search radius for the original
        # label. Many of them are moved during the Gaussian blur
        if not flag: skele2[i] = 0     
...
def count(pixel_size, skele, clusters):
    ...
    for val in xrange(1, max_val+1):
        ...
        # Nearest neighbour method  --- same result but costs less space 
        length = 0
...
def headerise(directory):
    ...
    path_list = search_for_file(directory, 'DBSCAN_Results.csv')
    for path in path_list:
        file_path = os.path.join(path, 'DBSCAN_Results.csv')
...
def summarise(directory, summary):              
    ...
    'NN_cluster_Ave':Series(summaryT[6]),
    'NN_cluster_SD':Series(summaryT[7]),
    'NN_localisation_Ave':Series(summaryT[8]),
    'NN_localisation_SD':Series(summaryT[9]),
    'NN_distance_Ave':Series(summaryT[10]),
    'NN_distance_SD':Series(summaryT[11]),
    'Length_Ave':Series(summaryT[12]),
    'Length_SD':Series(summaryT[13]),
    'Eccentricity_Ave':Series(summaryT[14]),
    'Eccentricity_SD':Series(summaryT[15]),
    'Density (Loc\\Length)':Series(summaryT[16])},
    columns = np.array(['Filepath', 'NumCluster', 'Precision_Ave', 
                        'Precision_SD', 'NumLocalisation_Ave', 
                        'NumLocalisation_SD',  'NN_cluster_Ave', 
                        'NN_cluster_SD', 'NN_localisation_Ave', 
                        'NN_localisation_SD', 'NN_distance_Ave',
                        'NN_distance_SD', 'Length_Ave', 'Length_SD',
                        'Eccentricity_Ave',  'Eccentricity_SD',
                        'Density (Loc\\Length)']))
