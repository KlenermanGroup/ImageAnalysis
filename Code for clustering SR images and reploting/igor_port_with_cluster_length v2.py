...
def user_input():
    ...
    directory = r"C:\Users\Daniel\Desktop\code test"
    pixel_size = 131.5
...
def nearest_neighbour(real_locs, directory):
    ...
    for cluster_num in xrange(int(np.amax(real_locs[:,15]))+1):
        ...
        num_locs = len(cluster_xy)
        if num_locs < 1:
            # use pdist - n^2 complexity
            ...
            # use Mathew's solution - (0)n complexity
            # first need to iterate through locs to get NN dist
            distances = []
            for a in xrange(num_locs):
                min_distance = 100 # nearest neighbour for that localisation
                xya = cluster_xy[a]
                for b in xrange(num_locs):
                    xyb = cluster_xy[b]
                    if list(xya) != list(xyb):
                        abs_dist = (xya-xyb)**2
                        dist = math.sqrt(abs_dist[0] + abs_dist[1])
                        if dist < min_distance:
                            min_distance = dist 
                distances.append(min_distance) # this list constains NN dist for each loc
            thresh_distance = np.mean(distances)*5 # 5*NN dist average for the cluster

            # now to iterate through again and see if distances are less than average distance
            nn_count = []
            for a in xrange(num_locs): 
                distances = []
                xya = cluster_xy[a]
                ...
                for dist in distances:
                    if dist < thresh_distance:
                        count += 1
                nn_count.append(count) # this list now contain num neighbours for each loc in the image
                
            # now as before
            neighbors_per_loc = np.array([nn_count])
            avg_nn_per_cluster.append(np.mean(neighbors_per_loc))
            # this section to save into all_localisation_information.txt
            # neighbours_per_loc is still in order of cluster_details
            for x in xrange(cluster_xy.shape[0]):
...
def count(pixel_size, skele, clusters):
    ...
    for val in xrange(1, max_val+1):
        ...
        dists = pdist(xy, metric='euclidean')           # Eric - pdist improvement; see v3 for the ndimage.measure.perimeter improvement
        dists = dists[dists < 2]
